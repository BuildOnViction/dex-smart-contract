...
const getProvider = (networkName) => {
  switch (networkName) {
    case "local":
      return new providers.Web3Provider(new Web3.providers.HttpProvider("http://localhost:8545"), { chainId: 8888 })      
    default:
...
const queryContractAddresses = () => {
  ...
  const contractAddressFile = path.join(__dirname, '../config/contractAddresses.json');
